...
for air in airports:
    possible_labels = submission_format[submission_format["airport"] == air]["config"].unique()
    ...
    for i in possible_labels:
        ...
        counter += 1
    cur_config = pd.read_csv(f"/codeexecution/data/{air}/{air}_airport_config.csv", parse_dates=["timestamp"])
    ...
    weather_file = f"/codeexecution/data/{air}/{air}_lamp.csv"
...
for i, r in opened.iterrows():
    ...
    try: 
        ...
        get_nearest = get_nearest.drop("timestamp", axis=1).drop("forecast_timestamp", axis=1)
    ...
    try:
        ...
        departure_features = latest_departure_report.values.tolist()[0][int(lookahead/30):int(lookahead/30)+4]
    except:
    ...
    try:
        ...
        arrival_features = latest_arrival_report.values.tolist()[0][int(lookahead/30):int(lookahead/30)+4]
    except:
    ...
    total_features = [r["airport"]] + weather_features + departure_features + arrival_features + cur_config_feature + [r["lookahead"]]
