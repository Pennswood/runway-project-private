...
def process_airports_departure_takeoff():
    opened = pd.read_csv("/codeexecution/data/partial_submission_format.csv", parse_dates=["timestamp"])
...
def process_airport_departure_takeoff(airport,start_time,end_time):
    ...
    landing_file = f"/codeexecution/data/{airport}/{airport}_tfm_estimated_runway_arrival_time.csv"
    take_off_file = f"/codeexecution/data/{airport}/{airport}_etd.csv"
    chunksize=10**6
