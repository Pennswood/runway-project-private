...
for cur_cross in range(3,temporal_cross):
   ...
   for air in airports:
      ...
      # weights = weights.iloc[:int(temporal_split/temporal_cross*cur_cross)].copy()
      ...
      stan = StandardScaler().fit(x_train,weights)
      x_train = stan.transform(x_train)
      # x_test = stan.transform(x_test)
      with open(f"{air}_standardize{cur_cross}_p.pkl", "wb") as file:
      ...
      LRG = linear_model.LogisticRegression(penalty='l2',C=0.001,solver = 'lbfgs', multi_class = 'multinomial', max_iter=75000000).fit(x_train, y_train, sample_weight=weights)
      ...
      #    for j in to_add:
      #       predicted_probabilities[i].insert(j, .001)
      ...
      with open(f"{air}_trained_model_temporal{cur_cross}_p.pkl", "wb") as file:
